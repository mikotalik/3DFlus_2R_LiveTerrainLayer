...
void main() {
  ...
  float hL = getHeightOf(sampler, P.xy - off.xz)*100.0;
  float hR = getHeightOf(sampler, P.xy + off.xz)*100.0;
  float hD = getHeightOf(sampler, P.xy - off.zy)*100.0;
  float hU = getHeightOf(sampler, P.xy + off.zy)*100.0;
  ...
  height = getHeightOf(sampler, vTexCoord);
  gl_Position += project_common_position_to_clipspace(vec4(0.0,0.0,height,1.0));
}
`;
