...
import LiveTerrainLayer from './LiveTerrainLayer';
import {PointCloudLayer} from '@deck.gl/layers';

const data = [{position: [0, 0],angle: 0, color: [255, 255, 255]}];
const initialViewState = {longitude: 0, latitude: 0, zoom: 15};

function generatePlaneMesh(width, height, sizeX, sizeY){
...
function getVectorsFromPositions(a){
  let o = [];

  for(let i = 0; i < a.length; i+=18){
    o[i/18] = [a[i]*10, a[i+1]*10, a[i+2]*10];
  }
...
function App() {
  ...
  const mesh = generatePlaneMesh(1000,1000,10,10) //2199, 1712, 10, 10

  const terrain = new LiveTerrainLayer({
    id: 'mesh-layer',
    data,
    mesh: mesh,
    texture: heightmap,
